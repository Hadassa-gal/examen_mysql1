...
```sql
SELECT em.nombre AS empleado, pa.nombre AS pais
FROM empleados em
JOIN sucursal su ON em.sucursalid = su.id
JOIN municipio mu ON su.municipioid = mu.id
JOIN departamento dep ON mu.depid = dep.id
JOIN pais pa ON dep.paisid = pa.id;
```
...
```sql
SELECT c.nombre, mu.nombre AS municipio, dep.nombre AS departamento, pa.nombre AS pais
FROM clientes c
JOIN municipio mu ON c.municipioid = mu.id
JOIN departamento dep ON mu.depid = dep.id
JOIN pais pa ON dep.paisid = pa.id;
```
...
```sql
SELECT em.nombre, em.puesto 
FROM empleados em
JOIN sucursal su ON em.sucursalid = su.id
WHERE (GROUP(em) BY(su.id)) > 1;
```
...
```sql
SELECT mu.nombre AS municipio, dep.nombre AS departamento, GROUP(em.id) BY(mu.id) AS total_empleados
FROM empleados em 
JOIN sucursal su ON em.sucursalid = su.id
JOIN municipio mu ON su.municipioid = mu.id
JOIN departamento dep ON mu.depid = dep.id;
...
FROM sucursal su
JOIN municipio mu ON su.municipioid = mu.id
WHERE (SELECT sucursalid FROM empleados) NOT NULL;
```
...
```sql
SELECT mu.nombre AS municipio, GROUP em.id BY em.municipioid AS total_empleados DESC
FROM empleados em 
JOIN sucursal su ON em.sucursalid = su.id
JOIN municipio mu ON su.municipioid = mu.id
LIMIT 5;
...
```sql
SELECT puesto, AVG(GROUP(salario) BY (puesto)) AS promedio, MAX(salario)
FROM  empleados;
```
...
```sql
SELECT su.nombre, GROUP(em.id)BY (em.sucursalid) AS total, AVG(COUNT(GROUP(em.salario) BY (em.sucursalid))) AS promedio
FROM empleados em
JOIN sucursal su ON em.sucursalid = su.id
WHERE AVG(COUNT(GROUP(em.salario) BY (em.sucursalid))) > 1800000 AND em.sucursal NOT NULL;
```
...
```sql
SELECT pa.nombre AS pais, COUNT(GROUP(COUNT(GROUP(COUNT(GROUP(c.id) BY(c.municipioid)))BY(mu.depid))BY(dep.paisid)) AS total_clientes
FROM clientes c
JOIN municipio mu ON c.municipioid = mu.id
JOIN departamento dep ON mu.depid = dep.id
JOIN pais pa ON dep.paisid = pa.id;
